...
func (s *GameOverScene) Render() {
	gameOverImage := canvas.NewImageFromURI(storage.NewFileURI("./assets/FinDelJuego.jpg"))
	gameOverImage.Resize(fyne.NewSize(200, 150))
	gameOverImage.Move(fyne.NewPos(300, 225))

	backgroundImage := canvas.NewImageFromURI(storage.NewFileURI("./assets/Fondo.jpg"))
